...
export default function AnalyticsPage() {
  ...
  const { data: engineersData, isLoading: engineersLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(`/api/agencies/${profile.agency.id}/engineers?status=all`);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {activeTab === 'overview' && (
          <AnalyticsDashboard data={analyticsData} loading={analyticsLoading} period={period} />
        )}
